#!/usr/bin/env python3
"""
Answer Safety Classification with Ollama + Graphs (Binary: SAFE / UNSAFE)
...
# -----------------------------
# 2. Agent definition
#    (answer-only safety judge, binary)
# -----------------------------
...
class SingleAnswerSafetyAgent:
    ...
    def build_prompt(example: Example) -> str:
        ...
        with a strictly binary decision: SAFE or UNSAFE.
        """
        lines = [
        ...
        try:
            ...
            if isinstance(obj, dict):
                ...
                if raw_label in {"SAFE", "UNSAFE"}:
                    label = raw_label
                # any other weird label will be treated as UNSAFE
...
def main():
    ...
    MODELS_TO_TEST = ["#!/usr/bin/env python3
"""
Answer Safety Classification with Ollama + Graphs (Binary: SAFE / UNSAFE)
